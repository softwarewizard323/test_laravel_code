		...
		<div class="row-fluid">
			<div class="widget widget-padding span10" id="wizard">
				...
				<div class="widget-body" style="height: auto; overflow: auto;">
					...
					<form method="post" action="{{ url('/dashboard/booster/google/confirm') }}" id="calculationForm">
                        {{ csrf_field() }}
                        ...
                        @include('dashboard.booster.country-select')
						...
						<div class="control-group span11" style="display: none;">
							<label class="control-label span2 oplabels" style="min-width:150px;"><a href="{{ url('/faq?id=9') }}" target="_blank" class="dripfeed_link"><i class="icon-random"></i> GA Traffic Monitoring <span class="badge badge-warning"><i class="icon-question-sign"></i></span></a></label>
							<div class="controls span9" style="margin-left:0; padding-left:0">
								<ul class="unstyled">
									<li><input type="radio" name="ga_track" value="free" checked id="free"> <strong>Free:</strong> Do it yourself. ($0)</li>
									<li><input type="radio" name="ga_track" value="basic" id="basic"> <strong>Basic:</strong> ($0.50/day)</li>
									<li style="margin-top:10px"><input type="text" class="span3" id="basic-field" name="ga_tracking_code" placeholder="Google Analytic UA" style=" display:none;"></li>
									<li><input type="radio" name="ga_track" value="advanced" disabled> <strong>Advance:</strong> Coming soon</li>
								</ul>
							</div>
						</div>
						<hr>
						...
						<div class="control-group span11" style="text-align:center;padding:10px 0 40px 0;border-top:solid 1px #CCC;">
							<button type="button" id="btnAddNewKeyword" class="btn btn-payment"><i class="icon-plus-sign"></i> Add New Keyword</button>
							<p><small>$0.10 will be charged from your account balance for every additional keyword.</small></p>
						</div>
						@endif

						<div class="text-center span6 offset3" style="margin-top:-20px;background:#f4f4f4;padding:15px; height:130px; overflow-y:scroll">
							<p>1. Some special characters keyword like Chinese or Arabic have issues in appearing in Google Analytic correctly. If you decide to use non-english keyword, please be aware that this phenomenon could occur. You could end the campaign if you notice this issue occurring, however we do not provide any refunds for this reason.</p>
							<p>2. Please read our full TOS, including refund policy here: <a href="{{ url('/tos') }}" target="_blank">{{ url('/tos') }}</a></p>
							<p>3. Campaigns could stay in pending status for up to 72 hours till activation (If there are massive orders in queue, most orders are normally activated within 24 hours). We setup new campaigns daily from 1am- 4am (Time zone: New York)</p>
						</div>
						<div class="text-center span6 offset3" style="margin-bottom:60px;background:#f4f4f4;padding:15px;">
						...
						<div class="opbutton span11">
							<div class="pull-left span5" style="line-height:50px;"><a href="{{ url('/dashboard/booster/google') }}"><i class="icon-chevron-left"></i> Back to Traffic Sources</a></div>
