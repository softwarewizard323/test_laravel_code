    ...
    <div class="row-fluid">
        <div class="widget widget-padding span12" style="padding-bottom:80px;">
            ...
            <tr>
                ...
                @if($filter == 'active') <th style="text-align:center;">Allowed End Date</th>@endif
            ...
            @foreach ($orders as $order)
                ...
                <td style="text-align:center">@if($order['type'] == 'Direct' || $order['type'] == 'Other Booster') @else google.{{$order['google_ext']}} @endif</td>
                <td style="text-align:center">@if($order['ga_track'] == 'basic') Basic @else Free @endif<br><i>{{ $order['ga_tracking_code'] }}</i></td>
                <td>{{ $order['website'] }}</td>
                <td style="text-align:center;">${{ $order->userBalance }}</td>
                <td style="text-align:center;">
                ...
                @if($filter == 'active')
                    <td style="text-align:center;">
                        {{ date('d-m-Y', strtotime($order['date'] . " +3 days")) }}
                    </td>
                @endif
                <td>
                ...
                <a href="{{ url('/admin/orders/update/delete', ['id' => $order['order_id']]) }}" class="btn btn-mini btn-danger" style="margin-top:3px;" onclick="return confirm('Are you sure want to delete this order?');"><i class="icon-remove-circle"></i> Delete</a>
                @if($filter == 'hidden')
                <a href="{{ url('/admin/orders/update/reveal', ['id' => $order['order_id']]) }}" class="btn btn-mini btn-success" style="margin-top:3px;" onclick="return confirm('Are you sure want to reveal this pending order?');"><i class="icon-remove-circle"></i> Reveal Pending</a>
                @else
                <a href="{{ url('/admin/orders/update/hide', ['id' => $order['order_id']]) }}" class="btn btn-mini btn-danger" style="margin-top:3px;" onclick="return confirm('Are you sure want to hide this pending order?');"><i class="icon-remove-circle"></i> Hide Pending</a>
                @endif
                @if ($order['status'] == '0')
                    <form action="{{ url('/admin/orders/update/start', ['id' => $order['order_id']]) }}" method="post">
                        {{ csrf_field() }}
                ...
                @elseif ($order['status'] == '3' && $order->End['status'] === 0)
                    <form action="{{ url('/admin/orders/update/confirm', ['id' => $order['order_id']]) }}" method="post">
                        {{ csrf_field() }}
                        <input name="username" value="{{ $order['username'] }}" type="hidden" />
                        <button class="btn btn-mini btn-danger" type="submit" style="margin-top:3px;" onclick="return confirm('Are you sure want to confirm this order');"><i class="icon-eject"></i> Confirm</button>
                    </form>
                @endif
                </td>
                </tr>
                <div id="keywords{{ $order['order_id'] }}" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    ...
                    <div class="modal-body">
                        <div class='span12'>
                            ...
                            @if(count(explode(';', $order['keywords'])) == count(explode(';', $order['quantity'])))
